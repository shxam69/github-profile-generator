...
class LogoProcessor:
    ...
    def process_logo(self, svg_path: Path, output_json: Path, point_count: int, scale_factor: float, padding: float, debug_img_name: str) -> None:
        ...
        if not paths:
            ...
            #
            # Draw ONLY the outlines (constellation effect).
            # This concentrates the limited particle budget on the recognizable silhouette
            # instead of wasting points filling large interior areas.
            #
            # A fixed 2px outline stroke does not carry enough unique pixels for
            # low-perimeter logos (simple wordmarks/glyphs) to fill point_count
            # without sampling the same pixel multiple times. Thicken the stroke
            # adaptively -- starting at the original 2px and increasing only as far
            # as needed -- until the rasterized outline has slightly more unique
            # pixels than point_count (target ~105%), so the later sampling step
            # can draw point_count DISTINCT positions instead of falling back to
            # sampling with replacement. High-detail logos (where 2px already
            # exceeds the target) are completely unaffected -- thickness stays at 2.
            target_unique_pixels = int(point_count * 1.05)
            max_thickness = 8  # safety cap against runaway thickening on pathological input
            thickness = 2
            mask = np.zeros((self.target_height, self.target_width), dtype=np.uint8)
            cv2.polylines(mask, scaled_polys, isClosed=True, color=255, thickness=thickness)
            y_idx, x_idx = np.where(mask == 255)

            while len(x_idx) < target_unique_pixels and thickness < max_thickness:
                thickness += 1
                mask[:] = 0
                cv2.polylines(mask, scaled_polys, isClosed=True, color=255, thickness=thickness)
                y_idx, x_idx = np.where(mask == 255)
            ...
            logger.info(
                f"{svg_path.name}: outline thickness={thickness}px yielded {len(x_idx)} unique pixels "
                f"(target >= {target_unique_pixels})"
            )
