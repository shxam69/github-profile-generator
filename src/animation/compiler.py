...
class AnimationCompiler:
    ...
    def compile(self, intro_json: Path, drift_json: Path, travellers_json: Path, graph_json: Path, output_json: Path) -> None:
        ...
        hold_end = hold_start + 2.0
        self._add_event("portrait_hold", hold_start, hold_end, all_dot_ids, easing="linear")
        ...
        for i, band_info in enumerate(drift_bands):
            ...
            if b_dots:
                self._add_event("drift_band", start, end, b_dots, easing="easeInOutQuad", metadata={"band_id": band_id})
                
        # 4. Traveller Depart (Portrait -> Logo 1)
        logger.info("Compiling traveller depart events...")
        traveller_depart_start = drift_start_time
        traveller_depart_end = traveller_depart_start + 2.0
        
        for t in traveller_paths:
            self._add_event("traveller_depart", traveller_depart_start, traveller_depart_end, [t["portrait_dot"]], easing="easeInOutExpo", metadata={
                "target_point": t["logo1_point"]
            })
            
        # 5. Logo Hold (Logo 1)
        logo1_hold_start = traveller_depart_end
        logo1_hold_end = logo1_hold_start + 2.0
        self._add_event("logo_hold", logo1_hold_start, logo1_hold_end, traveller_ids, easing="linear", metadata={"logo": 1})
        
        # 6. Logo Transition (Logo 1 -> Logo 2)
        logo1_trans_start = logo1_hold_end
        logo1_trans_end = logo1_trans_start + 1.5
        for t in traveller_paths:
            self._add_event("logo_transition", logo1_trans_start, logo1_trans_end, [t["portrait_dot"]], easing="easeInOutSine", metadata={
        ...
        logo2_hold_end = logo2_hold_start + 2.0
        self._add_event("logo_hold", logo2_hold_start, logo2_hold_end, traveller_ids, easing="linear", metadata={"logo": 2})
        
        # 8. Logo Transition (Logo 2 -> Logo 3)
        logo2_trans_start = logo2_hold_end
        logo2_trans_end = logo2_trans_start + 1.5
        for t in traveller_paths:
            self._add_event("logo_transition", logo2_trans_start, logo2_trans_end, [t["portrait_dot"]], easing="easeInOutSine", metadata={
        ...
        logo3_hold_end = logo3_hold_start + 2.0
        self._add_event("logo_hold", logo3_hold_start, logo3_hold_end, traveller_ids, easing="linear", metadata={"logo": 3})
        
        # 10. Traveller Return (Logo 3 -> Portrait)
        return_start = logo3_hold_end
        return_end = return_start + 2.0
        for t in traveller_paths:
            self._add_event("traveller_return", return_start, return_end, [t["portrait_dot"]], easing="easeInOutExpo", metadata={
        ...
        for i, band_info in enumerate(drift_bands):
            ...
            b_dots = [d for d in dot_ids if d not in traveller_set]
            if b_dots:
                self._add_event("portrait_restore", start, end, b_dots, easing="easeInOutQuad", metadata={"band_id": band_id})
